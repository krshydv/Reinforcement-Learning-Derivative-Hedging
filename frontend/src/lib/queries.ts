...
export function useTrainingRuns() {
  return useQuery({
    ...
    queryFn: () => fetchJson<{ run_id: string; experiment_name: string; algorithm: string; timesteps: number; status: string; created_at: string; updated_at: string }[]>("/training/"),
...
export function useBenchmarks() {
  return useQuery({
    queryKey: ["benchmarks"],
    queryFn: () =>
      fetchJson<{ pricing: Record<string, number>; risk: Record<string, number> | null; backtests: Record<string, Record<string, number>> | null; timings: Record<string, number> }>(
        "/benchmarks/run",
        {
          method: "POST",
          body: {
...
export function useExperiments() {
  return useQuery({
    ...
    queryFn: () => fetchJson<{ id: string; name: string; algorithm: string; status: string }[]>("/experiments/"),
    refetchInterval: 6000
  })
}

export function useStrategyBenchmarks() {
  return useQuery({
    queryKey: ["strategy-benchmarks"],
    queryFn: () => fetchJson<Record<string, Record<string, number>>>("/strategies/benchmarks"),
