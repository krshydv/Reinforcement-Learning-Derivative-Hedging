...
def simulate_market(config: MarketConfig) -> list[MarketState]:
    ...
    for t in range(1, steps):
        if config.regime_switching:
            regime = 0 if rng.random() < transition[regime, 0] else 1
        ...
        if config.model == "heston":
            ...
            theta = base_vol ** 2
            xi = 0.6
            z1 = correlated_normals(n_assets, corr, rng)
            z2 = correlated_normals(n_assets, corr, rng)
        ...
        if config.model == "jump":
            ...
            jump = rng.poisson(lam * dt, size=n_assets) * rng.normal(mu_j, sigma_j, size=n_assets)
        prices[t] = prices[t - 1] * np.exp((drift - 0.5 * diffusion ** 2) * dt + diffusion * np.sqrt(dt) * z1 + jump)
        if config.crash_prob and rng.random() < config.crash_prob:
            prices[t] *= 0.75
    series = []
...
def correlated_normals(n_assets: int, corr: np.ndarray, rng: np.random.Generator) -> np.ndarray:
    chol = np.linalg.cholesky(corr)
    return chol @ rng.normal(size=n_assets)
